...
- 構造的性質: adCatenam 出力の prefix/suffix を `List.IsPrefix` / `List.IsSuffix` で証明
- 代数的性質: adCatenamLista のリスト連結準同型性
- エスケープ安全性: evadeTextus/evadeArgumentum の出力中の特殊文字が全てエスケープ済みであること
...
- adCatenam 関数の構造的性質（prefix/suffix）を `startsWith`/`endsWith` で証明する
