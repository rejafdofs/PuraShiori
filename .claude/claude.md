...
- `Sakura/Theoremata.lean` に全称証明（∀ 量化）を追加する。具体例テストではなく、全ての入力について性質を証明すること
- 数値パラメータに上限/下限がある場合は依存型（`by omega`）で静的に検証する

### Sakura/Theoremata.lean の証明方針

具体例テスト（native_decide で特定の入力に対して検証）ではなく、全ての入力について ∀ 量化で証明する。

証明すべき性質の種類:
- 定数コンストラクタ: adCatenam の出力が期待文字列と一致（rfl）
- Bool パラメータ: `∀ (b : Bool), ...` で true/false 両方をカバー（`cases b <;> rfl`）
- Nat/Int パラメータ: `∀ (n : Nat), ...` で任意の数値をカバー（rfl）
- 有限列挙パラメータ: `∀ (d : DirectioAllineatio), ...` 等で全コンストラクタをカバー（`cases d <;> rfl`）
- String パラメータ: `∀ (s : String), ...` で任意の文字列をカバー（rfl）
- if/match 分岐を含むコンストラクタ: RHS に adCatenam の定義本体（let/if/match）をそのまま書いて rfl で証明する
- 構造的性質: adCatenam 出力の prefix/suffix を `List.IsPrefix` / `List.IsSuffix` で証明（`String.startsWith` は opaque なため使わない）
- 代数的性質: adCatenamLista のリスト連結準同型性
- エスケープ安全性: evadeTextus/evadeArgumentum の全称定義展開証明（rfl）。出力中の特殊文字が全てエスケープ済みであることの全称証明は `Elementa/Lemma.lean` に sorry 付きで配置（String.foldl 帰納法が必要なため後日証明）

### Sakura/Theoremata.lean のセクション構成

- §1: Bool パラメータの全称証明（audax, obliquus 等）
- §2: 定数コンストラクタの証明（SignumScopi, SignumImperii 等）
- §3: Nat/Int パラメータの全称証明（persona, mora 等）
- §4: adCatenamLista の代数的性質（空リスト、単一要素、連結準同型）
- §5: evadeTextus/evadeArgumentum の全称定義展開証明
- §6: 構造的性質（SignumScopi/SignumImperii/SignumFormae の prefix/suffix）
- §7: SignumFormae の列挙・String パラメータ全称証明
- §8: 残りカテゴリ（Fenestrae, Modorum, Mutationis, Soni, Retis, Animationis, Eventuum, Proprietatis, Inputi）の全称証明
- §8b: if/match 分岐を含むコンストラクタの全称証明

### Notatio/Verificatio.lean の役割

Verificatio.lean にはマクロ（scriptum!）固有の動作検証のみを置く。ランタイム関数の正しさは Theoremata.lean の全称証明で保証するため、native_decide による具体例テストは置かない。マクロ固有テストとは:
- 式埋込（{expr} 構文）
- 行跨ぎ改行自動挿入（マクロの行情報処理）
- 舊形式スコープ（\0, \1 等の糖衣構文）
- 裸テクストゥス・シングルクォート（マクロのパーサ挙動）
...
- 有限列挙型の全称証明は `intro x; cases x <;> rfl` で網羅する
- adCatenam 関数の構造的性質（prefix/suffix）を `List.IsPrefix` / `List.IsSuffix` で証明する（`String.startsWith` は opaque）
- if/match 分岐を含む adCatenam は RHS に定義本体を書いて rfl で証明する
- エスケープ関数の安全性は String.foldl → List.foldl 変換後に帰納法で示す
