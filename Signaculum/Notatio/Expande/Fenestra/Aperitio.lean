...
private def resolveOpenCumArgs (subCmd : String) (rest : Array Syntax) (stx : Syntax)
  ...
  | "dateinput" =>
    if h : rest.size = 5 then
      ...
      let res ← resolveCallbackum cb
      some <$> res.cumActione fun evStx _ => do
        `(Signaculum.Sakura.aperiInputumDiei $evStx (show String from $(⟨title⟩)) $(⟨y⟩) $(⟨m⟩) $(⟨d⟩))
  ...
  | "timeinput" =>
    if h : rest.size = 5 then
      ...
      let res ← resolveCallbackum cb
      some <$> res.cumActione fun evStx _ => do
        `(Signaculum.Sakura.aperiInputumTemporis $evStx (show String from $(⟨title⟩)) $(⟨hr⟩) $(⟨mi⟩) $(⟨se⟩))
  ...
  | "sliderinput" =>
    if h : rest.size = 5 then
      ...
      let res ← resolveCallbackum cb
      some <$> res.cumActione fun evStx _ => do
        `(Signaculum.Sakura.aperiInputumGradus $evStx (show String from $(⟨title⟩)) $(⟨mn⟩) $(⟨mx⟩) $(⟨init⟩))
  ...
  | "ipinput" =>
    if h : rest.size = 6 then
      ...
      let res ← resolveCallbackum cb
      some <$> res.cumActione fun evStx _ => do
        `(Signaculum.Sakura.aperiInputumIP $evStx (show String from $(⟨title⟩)) $(⟨a⟩) $(⟨b⟩) $(⟨c⟩) $(⟨d⟩))
  ...
  | "colorinput" =>
    if h : rest.size = 5 then
      ...
      let res ← resolveCallbackum cb
      some <$> res.cumActione fun evStx _ => do
        `(Signaculum.Sakura.aperiInputumColoris $evStx (show String from $(⟨title⟩)) $(⟨r⟩) $(⟨g⟩) $(⟨b⟩))
  ...
  | "inputbox" | "passwordinput" =>
    ...
    if h : rest.size = 2 ∨ rest.size = 3 then
      ...
      let res ← resolveCallbackum cb
      some <$> res.cumActione fun evStx _ => do
        `(Signaculum.Sakura.aperiInputum $modusStx $evStx
