...
| `excita f args*` | def ベースのイベントを發生させる（`@[term_parser]` 形式、`checkColGt` で次行 do-item を飲み込まない） |
| `insere f args*` | def ベースのイベントをスクリプトに埋め込む（同上） |
| `notifica f args*` | def ベースの NOTIFY を發生させる（同上） |
| `excitaPostTempus ms rep f args*` | def ベース遅延発火（同上） |
| `notificaPostTempus ms rep f args*` | def ベース遅延通知（同上） |
| `optioEventum titulus f args*` | def ベース事象付き選擇肢（同上） |
...
| `Notatio.Systema` | イベント・音響・動画・呼出・変更タグ（`\![raise,...]` `\![embed,...]` `\![notify,...]` `\![timerraise,...]` `\![timernotify,...]` `\![async,...]` `\![sound,...]` `\![anim,...]` 等） |
